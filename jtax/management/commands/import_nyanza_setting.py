...
#         print villageid
    ...
    def handle(self, *args, **options):
        ...
        misc_fees_path = os.sep.join([settings.ROOT_PATH, 'csv', 'nyanza_fees.csv'])
        misc_fees_csv = open(misc_fees_path, "rb")
        try:
            nyanza = District.objectsIgnorePermission.get(name='NYANZA', i_status="active")
            rows = csv.reader(misc_fees_csv, quotechar='"')
            for row in rows:
                if len(row) != 4:
                    import pdb
                    pdb.set_trace()
                    raise Exception("row too big %s" % row)
                category, name, value, term = row

                setting, created = Setting.objects.get_or_create(tax_fee_name='misc_fee',setting_name=category, sub_type=name, district=nyanza, defaults=dict(value=value.replace(',',''), valid_from=date.today(), description=term))
                if not created:
                    setting.value = value.replace(',','')
                    setting.description = term
                    setting.valid_from = date.today()
                    setting.save()
        finally:
            misc_fees_csv.close()
