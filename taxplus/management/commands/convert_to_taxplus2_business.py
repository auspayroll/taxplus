		...
		for b in Business.objects.all().order_by('id'):
			...
			for payfee in payfees:
				...
				if r.pk not in receipts:
					...
					deposit = BankDeposit.objects.create(account=account, bank_receipt_no=r.bank_receipt, bank=(r.bank or ''),
						date_banked=(r.paid_date or date(1900,1,1)), depositor_name=r.payer_name, amount=(r.amount or 0), status_id=r.status_id,
