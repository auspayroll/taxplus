...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		for line in lines:
			sector = Sector.objects.get(code=line['Sector Code'])
			if sector.name.lower() != line['Sector'].lower():
				if not sector.alias:
					sector.alias = ''
				if line['Sector'].lower() not in sector.alias.lower():
					sector.alias += ', ' + line['Sector'].title()
					sector.alias = sector.alias.strip(',')
					sector.save()

			cell = Cell.objects.get(code=line['Cell Code'])
			if cell.name.lower() != line['Cell'].lower():
				if not cell.alias:
					cell.alias = ''
			...
			if village.name.lower() != line['Village'].lower():
				if not village.alias:
					village.alias = ''
				if line['Village'].lower() not in village.alias.lower():
					village.alias += ' ,' + line['Village'].title()
					village.alias = village.alias.strip(',')
					village.save()
