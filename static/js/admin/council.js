...
function check_council_registration_form()
{
	name=$.trim($("#id_name").val());
	address=$.trim($("#id_address").val());
	if(name==''){
		$("#error").html('Please enter council name.');
		return false;		  
	}
	if(address==''){
		$("#error").html('Please enter council address.');
		return false;		  
	}
	if($("#id_boundary").html()=="")
	{
		$("#error").html("Please add boundary for this council!");
		return false;
	}
	else
	{
	...
	$.ajax({
			type:"post",
			url: "/admin/ajax/add_council/",
			data: {name: $("input#id_name").val(), address:$("#id_address").val(), boundary:$("#id_boundary").html()},
			success:function(data)
			{
				$("div#error").html("Council added successfully!");
				toggle();
				return;
			},
...
function checkCouncilConditions()
{
	name =  $.trim($("#id_name").val());
	address = $.trim($("#id_address").val());
	if(name=="")
	{
		$("#search_error").html("Please enter council name.");
		return false;
	}
	if(address="")
	{
		$("#search_error").html("Please enter address name.");
		return false;
	}
	return true;
}
...
function selectCouncilRepeater()
{
...
function selectCouncil()
{
	...
	$(document).ready(function(){
		...
		$('#id_name').click(function(){
			...
			{
				kw = $.trim($(this).val());
				querystring = "keyword="+kw+"&object_name=council";
				$.ajax(
					{
						type:"get",
