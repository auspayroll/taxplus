...
function check_district_registration_form()
{
	name=$.trim($("#id_name").val());
	

	if(name==''){
		$("#error").html('Please enter district name.');
		return false;		  
	}
	...
	$.ajax({
			type:"post",
			url: "/admin/ajax/add_district/",
			data: {name: $("input#id_name").val(), boundary:$("#id_boundary").html()},
			success:function(data)
			{
				$("div#error").html("District added successfully!");
				toggle();
				return;
			},
...
function checkDistrictConditions()
{
	name =  $.trim($("#id_name").val());
	if(name!=""){ return true;}
	else
	{
		$("#search_error").html("Please enter district name.");
		return false;
	}
}
...
function selectDistrictRepeater()
{
	...
	$('li[id^="li_district_"]').click(function(){
		...
		$('#id_district_id').val($(this).attr("districtid"));
	});
}
		...
		$('#id_name').click(function(){
			...
			else
			{
				...
				$.ajax(
					{
						type:"get",
						url: "/admin/ajax/search_district/",
						data: querystring,
						success:function(data)
						{
							if(data=="")
